...
import { looksLikeLatex, parse } from './internal/parse.js';

const EXTS = new Set(['tex', 'latex', 'ltx']);

export function latex(): Converter {
  return {
    id: 'latex',
    sniff(bytes: Uint8Array, hint?: ConvertHint): number {
      if (isPdf(bytes) || isOle(bytes) || isZip(bytes)) return 0;
      if (looksLikeLatex(bytes)) return 2;
