...
import { convertBook, looksLikeMobi } from './internal/parse.js';

const EXTS = new Set(['mobi', 'azw', 'azw3', 'prc']);

export function mobi(): Converter {
  return {
    id: 'mobi',
    sniff(bytes: Uint8Array, hint?: ConvertHint): number {
      if (looksLikeMobi(bytes)) return 2;
      if (hint?.path !== undefined && EXTS.has(fileExtension(hint.path) ?? '')) return 1;
      return 0;
    },
    convert(bytes: Uint8Array): ConvertResult {
      refuseForeign(bytes);
      return { markdown: convertBook(bytes) };
    },
  };
}
